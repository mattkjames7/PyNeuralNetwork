...
from Misc.ProgressBar import ProgressBar2
...
class NeuralNetwork (object):
	def __init__(self,Layers,Lambda=0.0,Ind=0,FT=False,ActFuncs='sigmoid',CostFunction='cross-entropy'):
		if isinstance(Layers,str):
			#load network
			self.LoadNetwork(Layers,Ind)
		else:
			self.Trained = False
	...
	def LoadNetwork(self,FileName,Ind=0):
		if os.path.isfile(FileName+'-{:02d}.nn'.format(Ind)) == False:
			print('file not found')
			return None
		f = open(FileName+'-{:02d}.nn'.format(Ind),'rb')
		self.Trained = np.fromfile(f,dtype='bool8',count=1)[0]
		self.L = np.fromfile(f,dtype='int32',count=1)[0]
		self.s = np.fromfile(f,dtype='int32',count=self.L)
		self.Lambda = np.fromfile(f,dtype='float32',count=1)[0]
		self.Range = np.fromfile(f,dtype='float32',count=1)[0]
		self.mt = np.fromfile(f,dtype='int32',count=1)[0]
		self.mc = np.fromfile(f,dtype='int32',count=1)[0]

		if self.mt > 0:
			self.Xt = np.fromfile(f,dtype='float32',count=self.mt*(self.s[0]+1)).reshape((self.mt,self.s[0]+1))
			self.yt = np.fromfile(f,dtype='float32',count=self.mt)
			self.TData = True
			self.CostFunction = ReturnUnrolledCostFunction(self.Xt,self.yt,self.s,self.Lambda)
			self.ThetaGradientFunction = ReturnUnrolledThetaGradientFunction(self.Xt,self.yt,self.s,self.Lambda)
		else:
			self.Xt = np.array([],dtype='float32')
			self.yt = np.array([],dtype='float32')
			self.TData = False
		if self.mc > 0:
			self.Xc = np.fromfile(f,dtype='float32',count=self.mc*(self.s[0]+1)).reshape((self.mc,self.s[0]+1))
			self.yc = np.fromfile(f,dtype='float32',count=self.mc)
			self.CVData = True
		else:
			self.Xc = np.array([],dtype='float32')
			self.yc = np.array([],dtype='float32')	
			self.CVData = False
			
				
		self.Theta = []
		for i in range(0,self.L-1):
			dim = [self.s[i+1],self.s[i]+1]
			self.Theta.append(np.fromfile(f,dtype='float32',count=dim[0]*dim[1]).reshape((dim[0],dim[1])))
		self.UTheta = UnrollTheta(self.Theta)
		
		self.nSteps = np.fromfile(f,dtype='int32',count=1)[0]
		self.nJ = np.fromfile(f,dtype='int32',count=1)[0]
		
		if self.nSteps > 0:
			self.Jt = np.fromfile(f,dtype='float32',count=self.nJ) 
			self.Jc = np.fromfile(f,dtype='float32',count=self.nJ) 
			self.Acct = np.fromfile(f,dtype='float32',count=self.nJ) 
			self.Accc = np.fromfile(f,dtype='float32',count=self.nJ) 
		else:
			self.Jt = np.array([],dtype='float32')
			self.Jc	= np.array([],dtype='float32')
			self.Acct = np.array([],dtype='float32')
			self.Accc= np.array([],dtype='float32')
	...
	def _CostFunction(self,a,y,Lambda=None):
...
#			#dl[:,0] = a[-1][:,0] - yk
	...
	def ParallelTrainNetwork(self,nEpoch=100,MiniBatchSize=10,Alpha=1.0,gTol=0.0,gTolEpochs=10,gTolMinEpochs=10,AlphaTestEpochs=1,AlphaRange=[1e-5,10.0]):
		...
		if self.nSteps == 0:
			...
			self.Jt[0] = self.CF(self._at[-1],self.yt,self.ThetaW,0.0)
			if self.CData:
				self._ForwardPropagate(self._zc,self._ac)	
				self.Jc[0] = self.CF(self._ac[-1],self.yc,self.ThetaW,0.0)
			else:
				self.Jc[0] = np.nan
			self.nSteps = 1
...
#					print('up')
	...
	def Train(self,Method='gradient-descent',**kwargs):
		...
		Methods = {	'gradient-descent':(self._CalculateStepGD,def_GD), #this one can do all simple batch,minibatch,stochastic GD
					'nesterov':(self._CalculateStepNesterov,def_Nesterov), #this will use the Nesterov momentum method - minibatches and SGD are enabled, but batch probably prefered
					'rprop':(self._CalculateStepRProp,def_RProp), #batch training using RProp method
					'rmsprop':(self._CalculateStepRMSProp,def_RMSProp)} #Stochastic/minibatch version of RProp
	...
	def _TrainGD(self,**kwargs):
		...
		self.Momentum = kwargs['Momentum']
		self.LocalGain = kwargs['LocalGain']
		self.LocalGainLimits = kwargs['LocalGainLimits']
		self.LocalGainSteps = kwargs['LocalGainSteps']
		...
		for i in range(0,nEpoch):
			...
			for j in range(0,nb):
				...
				y = self.yt[j*MiniBatchSize:(j+1)*MiniBatchSize]
				m = y.shape[0]
				self._CalculateStepGD(X,y,Alpha,m,n)
	...
	def _TrainNesterov(self,**kwargs):
		nEpoch = kwargs['nEpoch']
		MiniBatchSize = kwargs['MiniBatchSize']
		Alpha = kwargs['Alpha']
		gTol = kwargs['gTol']
		...
		for i in range(0,nEpoch):
			...
			for j in range(0,nb):
				...
				y = self.yt[j*MiniBatchSize:(j+1)*MiniBatchSize]
				m = y.shape[0]
				self._CalculateStepNesterov(X,y,Alpha,m,n)
	...
	def _TrainRProp(self,**kwargs):
		...
		self.LocalGainLimits = kwargs['LocalGainLimits']
		self.LocalGainSteps = kwargs['LocalGainSteps']
		self.RPropGainLimits = kwargs['RPropGainLimits']
		self.RPropGainSteps = kwargs['RPropGainSteps']
		#set up initial gains of 1.0
		self.GainW = []
		self.GainB = []
		for i in range(0,self.L-1):
		...
		for i in range(0,nEpoch):
			...
			for j in range(0,nb):
				...
				y = self.yt[j*MiniBatchSize:(j+1)*MiniBatchSize]
				m = y.shape[0]
				self._CalculateStepRProp(X,y,Alpha,m,n)
	...
	def _TrainRMSProp(self,**kwargs):
		...
		self.RPropGainLimits = kwargs['RPropGainLimits']
		self.RPropGainSteps = kwargs['RPropGainSteps']
		self.RMSPropDecay = kwargs['RMSPropDecay']
		self.MeanSqW = []
		self.MeanSqB = []
		for i in range(0,self.L-1):
			self.MeanSqW.append(np.ones(self.ThetaW[i].shape))
		...
		for i in range(0,nEpoch):
			...
			for j in range(0,nb):
				...
				y = self.yt[j*MiniBatchSize:(j+1)*MiniBatchSize]
				m = y.shape[0]
				self._CalculateStepRMSProp(X,y,Alpha,m,n)
	...
	def SaveNetwork(self,FileName='',ind=0):
		if FileName == '':
			print('Please enter valid file name')
			return
	
		f = open(FileName+'-{:02d}.nn'.format(ind),'wb')
		np.bool8(self.Trained).tofile(f)
		np.int32(self.L).tofile(f)
		np.int32(self.s).tofile(f)
		np.float32(self.Lambda).tofile(f)
		np.float32(self.Range).tofile(f)
		np.int32(self.mt).tofile(f)
		np.int32(self.mc).tofile(f)


		if self.mt > 0:
			self.Xt.astype('float32').tofile(f)
			self.yt.astype('float32').tofile(f)
		if self.mc > 0:
			self.Xc.astype('float32').tofile(f)
			self.yc.astype('float32').tofile(f)
		
		for i in range(0,self.L-1):
			self.Theta[i].astype('float32').tofile(f)
		
		
		np.int32(self.nJ).tofile(f)
		np.int32(self.nSteps).tofile(f)
		if self.nJ > 0:
			self.Jt.astype('float32').tofile(f)
			self.Jc.astype('float32').tofile(f)
			self.Acct.astype('float32').tofile(f)
			self.Accc.astype('float32').tofile(f)
	...
	def PlotCost(self,fig=None,maps=[1,1,0,0],xrnge=None,yrnge=None):
		...
		ax.plot(self.Jt[0:self.nSteps],color=[1.0,0.0,0.0],label='Training Set Cost')
		...
		if xrnge is None:
			xrnge = [0,self.nSteps]
		if yrnge is None:
			yrnge = [0,np.nanmax([self.Jt,self.Jc])]
